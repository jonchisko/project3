...
func test_prepend_multiple_messages() -> void:
	# Arrange
	var object_under_test: MessageManager = MessageManager.new()
	var messages: Array[Dictionary] = [{"role": "test", "content": "test_message"}, {"role": "system", "content": "system_message"}]
	...
	for index in messages.size():
		...
		var expected_role = messages[messages.size() - index - 1]["role"]
		var expected_message = messages[messages.size() - index - 1]["content"]
...
func test_prepend_message_dictionary() -> void:
	...
	# Act
	object_under_test.prepend_message_dictionary({"role": "test", "content": "test_message"})
